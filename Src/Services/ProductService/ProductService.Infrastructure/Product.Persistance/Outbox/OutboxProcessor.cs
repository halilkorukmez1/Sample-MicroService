...
using Product.Infrastructure.Events.OutboxSendEvent;
...
internal sealed class OutboxProcessor : IHostedService
{
    ...
    public OutboxProcessor(IServiceScopeFactory serviceScopeFactory, IOptions<OutboxOptions> options, IEventBus eventBus)
    {
    ...
    private void SendOutboxMessages(object state) => _ = Process();
        ...
        using (var scope = _serviceScopeFactory.CreateScope())
        {
            ...
            try
            {
                foreach (var messageId in messageIds)
                {
                    ...
                    _eventBus.Publish(new WriteOutboxToElasticEvent( message.Data ));
